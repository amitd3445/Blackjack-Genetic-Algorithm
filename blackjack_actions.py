...
class Game:
    ...
    def restart_table(self):
        ...
        if self.deck.number_of_cards < self.deck.minimum_number_of_cards:
            self.deck.shuffle()
        return
    
class Series:
    def __init__(self, number_of_hands, decision_matrix, number_of_decks = 6, minimum_number_of_cards = 20, dealer_max_total_to_hit = 17):
        self.game = Game(decision_matrix, number_of_decks, minimum_number_of_cards, dealer_max_total_to_hit)
        self.number_of_hands = number_of_hands
        self.cummulative_results = 0
    
    def start_series(self):
        for _ in range(self.number_of_hands):
            self.game.one_round()
            self.cummulative_results += self.game.result
            self.game.restart_table()
